...
export class CallbackHandler {
  ...
  setupCallbacks() {
    ...
    this.bot.on("callback_query", async (callbackQuery) => {
      ...
      try {
        if (data.startsWith("save_") || data.startsWith("cancel_")) {
          ...
          if (!post) {
            ...
            await this.bot.sendMessage(
              chatId,
              "Ошибка: напоминание не найдено или устарело",
              {
                reply_markup: {
                  inline_keyboard: [
        ...
        switch (true) {
          case data === "open_web_app":
            await bot.sendMessage(chatId, "Открываем приложение...", {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
          ...
          case /^delete_(.+)$/.test(data): {
            ...
            if (!remindId || remindId === "undefined") {
              ...
              await this.bot.sendMessage(
                chatId,
                "❗ Невозможно удалить это напоминание — отсутствует идентификатор."
              );
              break;
            }
          ...
          case /^put_(.+)$/.test(data): {
            ...
            if (!remindId || remindId === "undefined") {
              ...
              await this.bot.sendMessage(
                chatId,
                "❗ Невозможно изменить это напоминание — отсутствует идентификатор."
              );
              break;
            }

            await this.bot.sendMessage(chatId, "Что вы хотите изменить?", {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: "📝 Изменить содержание",
                      callback_data: `change_content_${remindId}`,
                    },
                  ],
                ],
              },
          ...
          case /^change_content_(.+)$/.test(data): {
            ...
            if (!remindId) {
              await this.bot.sendMessage(
                chatId,
                "❗ Невозможно изменить содержание — отсутствует идентификатор."
              );
              break;
            }
  ...
  storePost(post) {
    ...
    if (this.postsStorage.has(key)) {
      console.log(
        `Пост ${key} уже есть в хранилище, повторное сохранение пропущено`
      );
      return;
    }
    ...
    console.log(
      "Текущее содержимое хранилища:",
      Array.from(this.postsStorage.keys())
    );
  }
}
